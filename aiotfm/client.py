...
class Client:
	...
	def __init__(
		...
		self.authkey: int = 0
		self.pid: int = 0
		self.verifycode: int = 0

		self.auto_restart: bool = auto_restart
		self.bot_role: bool = False
	...
	async def handle_packet(self, connection: Connection, packet: Packet) -> bool:
		...
		elif CCC == (5, 44): self.verifycode = packet.readShort()
		...
		if CCC == (5, 21): # Joined room
			...
			os_info = Packet.new(28, 17).writeString('en').writeString('Windows')
			...
			c_int1 = packet.read32()
			await self.sendnobot(c_int1)
	...
	async def sendHandshake(self):
		...
		packet.writeString('Desktop').writeString('x').write32(8125).writeString('')
		packet.writeString('74696720697320676f6e6e61206b696c6c206d7920626f742e20736f20736164')
		packet.writeString("A=t&SA=t&SV=t&EV=t&MP3=t&AE=t&VE=t&ACC=t&PR=t&SP=f&SB=f&DEB=f&V=LNX 32,0,0,182&M=Adobe Linux&R=1920x1080&COL=color&AR=1.0&OS=Linux&ARCH=x86&L=en&IME=t&PR32=t&PR64=t&LS=en-US&PT=Desktop&AVD=f&LFD=f&WD=f&TLS=t&ML=5.1&DP=72")
		packet.write32(0).write32(25175).writeString('')
	...
	async def start(self, client_id: Optional[str] = None, **kwargs):
		...
		while True:
			...
			try:
				...
				logger.error('[-] Keys outdated, please update them!')
	...
	async def sendnobot(self, code):
		p1= Packet()
		p1.write16(45103)
		p1.write32(code).writeUTF('----').write32(code).writeUTF("-//\\\n/").write32(81).write16(108).write16(888)#.write8(16)
		...
		def djb(s):
			...
			hash = 5381
			for i in range(20): 
				hash = (hash << 5) + hash + (self.keys.packet[i] + s[(i % s_len)])
			else:
				for i in range(20):
				    ...
				    return buf
		p1.cipher(djb(str(code).encode()))

		await self.main.send(p1)
	...
	async def login(self, username: str, password: str, encrypted: bool = True, room: str = '1'):
		...
		packet.write16(18).write8(self.verifycode).writeString('')
